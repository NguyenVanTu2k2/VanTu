...
export const TodoList = () => {
  ...
  const { data: todos = [] } = api.todo.getAll.useQuery({
    statuses: selectedTab === 'all' ? ['completed', 'pending'] : [selectedTab],
  })
  ...
  return (
    <div>
      <Tabs
        defaultValue="all"
        onValueChange={(value) =>
          setSelectedTab(value as 'all' | 'pending' | 'completed')
        }
      >
        <TabsList className="flex space-x-4 pb-10 ">
          <TabsTrigger
            value="all"
            className={`rounded-lg rounded-full border border-gray-300 px-4 py-2  ${
              selectedTab === 'all'
